...
class contour2mask:
    ...
    def __main__(self):
        ...
        if self.study == 'CKD2':
            ...
            if '007' in self.filename or '011' in self.filename:
        ...
        #plt.imshow(np.mean(gt_mag[center[0] - 32: center[0] + 32, center[1] - 32: center[1] + 32, :], axis = -1)*np.mean(mask[center[0] - 32: center[0] + 32, center[1] - 32: center[1] + 32, :], axis = -1), cmap = 'gray')
            ...
            #filename = self.filename.replace('mat','vtk')
            
            #filename = filename.replace('msk','mag')
            
            #self.array2vtk(gt_mag, filename, self.dest_path, origin, spacing)
